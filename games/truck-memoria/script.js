...
let state = { cards: [], flipped: [], matched: 0, moves: 0, timerId: null, seconds: 0, locked: false };
...
function buildBoard() {
  ...
  state.cards.forEach((symbol) => {
    ...
    card.innerHTML = `<div class="inner"><div class="face front"></div><div class="face back">${symbol}</div></div>`;
  ...
  if (state.flipped.length === 2) {
    ...
    if (a.dataset.symbol === b.dataset.symbol) {
      ...
      if (state.matched === SYMBOLS.length) {
        clearInterval(state.timerId);
        setTimeout(() => alert(`Frota completa! ${state.moves} jogadas em ${formatTime(state.seconds)}.`), 300);
      }
    } else {
      state.locked = true;
      setTimeout(() => {
